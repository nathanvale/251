...
function getDocPaths(module: any, packageName: string) {
  return Object.keys(module).map(name => ({
    name,
    docsPath: packageName,
  }));
}
const componentPaths = [
  ...getDocPaths(odsCore, "ods-core"),
...
export const Documentation = () => {
  ...
  const componentsByCategory = groupBy(
    componentPaths
      ...
      .map(({name, docsPath}) => {
        const docs: ComponentDocs = require(`../../../../packages/${docsPath}/src/${name}/${name}.docs.tsx`)
          .docs;
        return {name, ...docs};
      }),
                ...
                <Stack space="large">
                  <MenuSectionList
                    ...
                    items={[
                      ...
                      },
                      {
                        ...
                        path:
                          "https://app.zeplin.io/project/5a04e5b2e5f0b90b17771a32",
                        external: true,
                        onClick: () => setMenuOpen(isComponentsHome),
                      },
                    ]}
                  ...
                  <MenuSectionList
                    ...
                    items={Object.keys(components)
                      .filter(
                        x =>
                          !/^(Icon|use|BoxRenderer|OriginThemeProvider)/.test(
                            x,
                          ),
                      )
                      .sort()
